"""
Sample quickstart guide for Azure Managed Instance for Apache Cassandra
"""
...
cluster = Cluster([cfg.config['contactPoint']], port = cfg.config['port'], auth_provider=auth_provider,ssl_context=ssl_context, protocol_version=4)
session = cluster.connect()
#</authenticateAndConnect>

#<createKeyspace>
print ("\nCreating Keyspace")
execute_command('CREATE KEYSPACE IF NOT EXISTS uprofile WITH replication = {\'class\': \'NetworkTopologyStrategy\', \''+cfg.config['datacenter-name']+'\' : \'3\' }');
#</createKeyspace>
